...
const DATASET = { N: WORDS_N, A: WORDS_A, B: WORDS_B, C: WORDS_C};
...
// Gabungkan supaya mudah diakses seragam:
const SENTENCES = {
  ...SENTENCES_N,
  ...SENTENCES_A,
  ...SENTENCES_B,
...
const GENERIC_TEMPLATES = [
  (w) =>
    `${w} banyak ditemukan dalam kegiatan sehari-hari dengan fungsi yang beragam.`,
  (w) =>
    `Di beberapa daerah, ${w.toLowerCase()} memiliki makna khusus dalam budaya setempat.`,
  (w) =>
    `Contoh penggunaan ${w.toLowerCase()} dapat diamati langsung di lingkungan sekitar.`,
  (w) =>
    `Pembahasan mengenai ${w.toLowerCase()} sering muncul di media dan bahan ajar.`,
  (w) =>
    `Pemahaman tentang ${w.toLowerCase()} membantu menambah wawasan dan kosakata.`,
  (w) =>
    `Riset sederhana dapat dilakukan untuk melihat pengaruh ${w.toLowerCase()} pada aktivitas.`,
  (w) =>
    `Dokumentasi yang baik membuat data terkait ${w.toLowerCase()} lebih mudah dianalisis.`,
  (w) =>
    `Kebijakan tertentu dapat mempengaruhi cara masyarakat memanfaatkan ${w.toLowerCase()}.`,
];

function kalimatFor(word) {
  const W = word.toUpperCase();
  if (SENTENCES[W]) return SENTENCES[W]; // ← cek ke gabungan semua huruf
  // kalau memang TIDAK ada di kamus, baru pakai template acak
